...
import { ethereumTokens, bscTokens, blastSepoliaTokens } from "@pancakeswap/tokens";
...
const chainName: { [key: number]: string } = {
  ...
  [ChainId.BASE]: "base",
  [ChainId.OPBNB]: "opbnb",
  [ChainId.BLAST_TESTNET]: "blast-sepolia",
};

// TODO: move to utils or token-list
export const getTokenListBaseURL = (chainId: number) =>
  `https://yantra-assets.netlify.app/images/${chainName[chainId]}`;

export const getTokenListTokenUrl = (token: Token) =>
  chainName[token.chainId]
    ? `https://yantra-assets.netlify.app/images/${token.chainId === ChainId.BSC ? "" : `${chainName[token.chainId]}/`}${
        token.address
      }.png`
    : null;
...
const commonCurrencySymbols = [
  ethereumTokens.usdt,
  ethereumTokens.usdc,
  blastSepoliaTokens.yantra,
  blastSepoliaTokens.usdt,
  bscTokens.cake,
  ethereumTokens.wbtc,
  ethereumTokens.weth,
  NATIVE[ChainId.BSC],
...
export const getCommonCurrencyUrlBySymbol = memoize(
  (symbol?: string): string | undefined =>
    symbol && commonCurrencySymbols.includes(symbol)
      ? `https://yantra-assets.netlify.app/images/symbol/${symbol.toLocaleLowerCase()}.png`
      : undefined,
