...
import { LAUNCHPAD_ADDRESS } from 'config/constants/yantra'
...
const description = {
  allocate: {
    ...
    description: "Allocate xYANTRA to this plugin to earn perks from our launchpad's partnering protocols",
  },
  deallocate: {
    title: 'Deallocate from launchpad',
    description: "Deallocate xYANTRA from this plugin to stop earning from our launchpad's partnering protocols",
  },
}
...
const AllocateModal: React.FC<React.PropsWithChildren<AllocateModalProps>> = ({
  ...
  const handleApprove = async () => {
    ...
    const receipt = await fetchWithCatchTxError(() => {
      return handleApproveUsage(getParseUnits(userData.xYantraBalance))
    })
    setLoading(false)
    if (receipt?.status) {
      refetchAllowance()
  ...
  return (
    <Modal title={description[tab].title} onDismiss={onDismiss}>
        ...
        <EBox style={{ marginTop: '8px' }}>
          <Inner>
            <Flex flexDirection="column" flexGrow={1} justifyContent="center">
              ...
              />
              <Balance>
                BALANCE: {tab === 'allocate' ? `${userData.xYantraBalance} xYANTRA` : `${userData.allocated} xYANTRA`}{' '}
              </Balance>
            </Flex>
            <ECircleButton onClick={() => setAmount(tab === 'allocate' ? userData.xYantraBalance : userData.allocated)}>
              MAX
            </ECircleButton>
          </Inner>
        </EBox>
      ...
      {tab === 'allocate' && amount && amount <= userData.xYantraBalance ? (
      ...
      ((tab === 'deallocate' && amount > userData.allocated) ||
        (tab === 'allocate' && amount > userData.xYantraBalance)) ? (
