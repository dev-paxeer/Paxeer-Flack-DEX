...
const description = {
  allocate: {
    ...
    description: "Allocate xYANTRA to this plugin to earn a share of our protocol's revenue",
  },
  deallocate: {
    title: 'Deallocate from dividends',
    description: "Deallocate xYANTRA from this plugin to stop earning a share of our protocol's revenue",
  },
}
...
const AllocateModal: React.FC<React.PropsWithChildren<AllocateModalProps>> = ({
  ...
  const handleApprove = async () => {
    ...
    const receipt = await fetchWithCatchTxError(() => {
      return handleApproveUsage(getParseUnits(data.xYantraBalance))
    })
    setLoading(false)
    if (receipt?.status) {
      refetchAllowance()
  ...
  return (
    <Modal title={description[tab].title} onDismiss={onDismiss} style={{ minWidth: '360px' }}>
      <CardContent>
        ...
        <EBox>
          <Inner>
            <Flex flexDirection="column" flexGrow={1} justifyContent="center">
              ...
              />
              <Balance>
                BALANCE: {tab === 'allocate' ? `${data.xYantraBalance}xYANTRA` : `${data.manualAllocation}YANTRA`}{' '}
              </Balance>
            </Flex>
            <ECircleButton onClick={() => setAmount(tab === 'allocate' ? data.xYantraBalance : data.manualAllocation)}>
              MAX
            </ECircleButton>
          </Inner>
        </EBox>

        {tab === 'allocate' && amount && amount <= data.xYantraBalance ? (
        ...
        {tab === 'deallocate' && amount && amount <= data.allocated ? (
          <Flex flexDirection="column" mt="12px" style={{ gap: '8px' }}>
            ...
            <Flex justifyContent="space-between">
              ...
              <Flex style={{ fontSize: '10px' }} alignItems="center">
                {(amount * dashboardData.deAllocationFee) / 100} xYANTRA
              </Flex>
            </Flex>
          </Flex>
        ) : (
        ...
        {amount &&
        ((tab === 'deallocate' && amount > data.allocated) || (tab === 'allocate' && amount > data.xYantraBalance)) ? (
        ...
        <Flex justifyContent="center" mt={10}>
          <EButton
            ...
            disabled={
              ...
              (tab === 'allocate' && amount > data.xYantraBalance)
            }
            isLoading={isLoading}
          >
            {isLoading ? (
