...
const BoosterModal: React.FC<React.PropsWithChildren<BoosterModalProps>> = ({ onDismiss, data, onRefetchData }) => {
  ...
  const { balance: xYantraBalance } = useTokenBalance(XYANTRA_ADDRESS)
  ...
  return (
    <Modal title={`${data.name} ${data.type} fsNFT`} onDismiss={onDismiss}>
      ...
      <Text fontSize="14px" color="gray" textAlign="center">
        Allocate xYANTRA to your fsNFT for more yield
      </Text>
      ...
      <EBox style={{ marginTop: '8px' }}>
        <Inner>
          <Flex flexDirection="column" flexGrow={1} justifyContent="center">
            ...
            <Balance>
              BALANCE:{' '}
              {tab === 'boost' ? `${getFormattedUnits(xYantraBalance)} xYANTRA` : `${data.boostPoints} xYantra`}{' '}
            </Balance>
          </Flex>
          <ECircleButton
            onClick={() => setAmount(tab === 'boost' ? getFormattedUnits(xYantraBalance) : data.boostPoints)}
          >
            MAX
          </ECircleButton>
        </Inner>
      ...
      {amount &&
      ((tab === 'boost' && amount <= getFormattedUnits(xYantraBalance)) ||
      ...
      <FlexGap justifyContent="space-around" mt={12} gap="8px">
        ...
        {tab === 'boost' ? (
          <EButton
            ...
            disabled={!amount || isLoading || amount > data.xYantraBalance}
            mt={5}
          >
            {isLoading ? (
