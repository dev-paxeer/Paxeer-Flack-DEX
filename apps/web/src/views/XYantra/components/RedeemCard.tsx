...
import { getFormattedUnits, getParseUnits } from 'utils/yantraHelper'
...
const RedeemCard = ({ setRefresh, onRefetchData }) => {
  ...
  const xYantraBalance = useTokenBalance(XYANTRA_ADDRESS)
  ...
  return (
    <CardContent>
      <Flex flexDirection="column">
        <RewardTitle>Redeem xYANTRA</RewardTitle>
        <RewardValue>
          Redeem your xYANTRA back into YANTRA over a vesting period of {limitDuration.minDuration} days
        </RewardValue>
      </Flex>
      ...
      <EBox>
        <Inner>
          <Flex flexDirection="column" flexGrow={1} justifyContent="center">
            ...
            />
            <Balance>BALANCE: {getFormattedUnits(xYantraBalance.balance)} xYANTRA</Balance>
          </Flex>
          <ECircleButton onClick={() => setAmount(getFormattedUnits(xYantraBalance.balance))}>MAX</ECircleButton>
        </Inner>
      </EBox>

      {amount && amount > 0 && amount <= getFormattedUnits(xYantraBalance.balance) ? (
        <Flex flexDirection="column">
          ...
          <Flex justifyContent="space-between">
            ...
            <Text fontSize="13px">{getRedeemYantra()} Yantra</Text>
          </Flex>
        </Flex>
      ) : (
        <></>
      ...
      {amount > getFormattedUnits(xYantraBalance.balance) && (
