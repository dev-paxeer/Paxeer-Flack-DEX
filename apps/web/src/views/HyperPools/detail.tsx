...
const HyperPoolDetail = () => {
  ...
  const { apr: fsNFTApr } = useFsNFTAprs(poolData?.nftPoolAddress, yantraPrice, lpPrice)
  ...
  const [onPresentHarvestModal] = useModal(
    <HarvestModal
      ...
      fsNFTs={fsNFTs}
      rewards={rewards}
      priceData={{ rewardsToken1Price, rewardsToken2Price, lpPrice, yantraPrice }}
    />,
  )
  ...
  return (
    <Page>
      ...
      >
        <EPageHeader
          pageName={`HyperPool`}
          subTitle={poolData ? `${poolData?.pairName} ${poolData?.isStable ? 'Stable' : 'V2'}` : ''}
        />
