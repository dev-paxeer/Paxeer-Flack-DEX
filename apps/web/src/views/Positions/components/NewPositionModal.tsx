...
import { YANTRA_ADDRESS, NFT_POOL_FACTORY_ADDRESS } from 'config/constants/yantra'
...
export const NewPositionModal: React.FC<React.PropsWithChildren<NewPositionModalProps>> = ({ onDismiss }) => {
  ...
  const { lpPrice: v2LpPrice } = useLpPrice(
    lpToken !== undefined && pairData && pairData.length > 0
      ? pairData[lpToken === undefined ? 0 : lpToken].token0.id ===
        STABLE_COIN[chainId ?? ChainId.BLAST_TESTNET].address.toLowerCase()
  ...
  const aprData = useFsNFTAprs(poolAddress, yantraPrice, lpPrice, '0', (duration * DAY_IN_SECONDS).toString(), 0)
