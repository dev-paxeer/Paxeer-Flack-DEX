...
const getTokenLogoURL = memoize(
  (token?: Token) => {
    if (token && mapping[token.chainId] && isAddress(token.address)) {
      return `https://yantra-assets.netlify.app/images/${token.chainId}/${safeGetAddress(token.address)}.png`
    }
    return null
  },
...
export const getTokenLogoURLWithSymbol = (tokenSymbol?: string) => {
  if (tokenSymbol) return `https://yantra-assets.netlify.app/images/symbol/${tokenSymbol}.png`
  return null
}
