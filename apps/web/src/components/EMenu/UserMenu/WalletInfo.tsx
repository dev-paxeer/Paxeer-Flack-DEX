...
const WalletInfo: React.FC<WalletInfoProps> = ({ hasLowNativeBalance, onDismiss }) => {
  ...
  const { balance: wYantraBalance, fetchStatus: wYantraFetchStatus } = useTokenBalance(YANTRA_ADDRESS)
    ...
    <>
      ...
      {!isBSC && chain && (
        <Box mb="12px">
          ...
          {wYantraBalance && wYantraBalance.gt(0) && (
            <Flex alignItems="center" justifyContent="space-between">
              <Text color="textSubtle" fontSize="12px">
                YANTRA {t('Balance')}
              </Text>
              {wYantraFetchStatus !== FetchStatus.Fetched ? (
                <Skeleton height="22px" width="60px" />
              ) : (
                <Text fontSize="12px">{getFullDisplayBalance(wYantraBalance, 18, 6)}</Text>
              )}
            </Flex>
          )}
        </Box>
