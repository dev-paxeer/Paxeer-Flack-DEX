...
// Only attempt real uploads if all Sentry credentials are present
...
const sentryWebpackPluginOptions =
  process.env.VERCEL_ENV === 'production'
    ? hasSentryCreds
      ? {
          // Production + credentials available => real upload
          silent: false,
          validate: true,
          hideSourceMaps: true,
        }
      : {
          // Production but missing credentials => do not fail the build
          silent: true,
          validate: false,
          dryRun: true,
          hideSourceMaps: true,
    ...
    : {
        // Non-production builds should never upload
        hideSourceMaps: true,
        silent: true,
        dryRun: true,
      }
